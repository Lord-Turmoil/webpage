(function() {
    function w() {
        ...
        // h.fillStyle = "rgba(255, 255, 255," + v + ")";
        // h.fillStyle = "rgba(128, 128, 128," + v + ")";
        h.fillStyle = "rgba(0, 0, 0," + v + ")";
        h.fillRect(0, 0, f, k);
        ...
        for (var a = f / 1.15, l = f / 8, p = f / 2, r = Math.random, F = Math.abs, w = Math.sqrt, A = Math.cos, B = Math.sin, C = Math.atan2, G = 1350; G--;) {
            var e = H[G],
                ...
                q = A(m),
                m = B(m);
            if (z && n < p) var u = 14 * (1 - n / p),
                d = d + (q * u + .5 - r()),
                b = b + (m * u + .5 - r());
            n < a && (u = (1 - n / a) * f * .0014, d -= q * u, b -= m * u);
            n < l && (n = (1 -
                n / l) * f * 2.2E-4, d += D * n, b += E * n);
            d *= .96;
            b *= .96;
            F(d);
            F(b);
            ...
            d.pos3D = b;
            e.pos3D.z -= 200; - 299 > e.pos3D.z || (d = 300 / (300 + e.pos3D.z), b = e.pos3D.x * d + f / 2, c = e.pos3D.y * d + k / 2, h.fillStyle = e.color, h.beginPath(), h.arc(b, c, d * 2, 0, I, !0), h.closePath(), h.fill())
        }
    }
